...
macro_rules! molt_ok {
    () => (
        Ok($crate::value::Value::empty())
    );
    ($arg:expr) => (
        Ok($crate::value::Value::from($arg))
    );
    ($($arg:tt)*) => (
        Ok($crate::value::Value::from(format!($($arg)*)))
    )
}
...
macro_rules! molt_err {
    ($arg:expr) => (
        Err($crate::types::Exception::molt_err($crate::value::Value::from($arg)))
    );
    ($($arg:tt)*) => (
        Err($crate::types::Exception::molt_err($crate::value::Value::from(format!($($arg)*))))
    )
}
