...
impl <'a> Wave<'a> {
    ...
    pub fn ui(self, ui: &mut Ui) -> Response {
        ...
        let total_wave_width = scale * wave_data.len() as f32;
        ...
        let dx = 1.0;
        let dy = 0.9;
        let first_data_ix = (view_range.start() / 32.0 / scale).floor() as usize;
        let last_view_ix = (view_range.end() / 32.0 / scale).ceil() as usize;
        let last_data_ix = std::cmp::min(wave_data.len(), last_view_ix);
        if last_data_ix <= first_data_ix {
            return response;
        }
        let mut x = first_data_ix as f32;
        if wave_data[first_data_ix] {
            last_high = true;
        ...
        let mut shapes = vec![];
        shapes.push(Shape::line(pts.iter().map(|v| pos_from_val(*v, rect, wave_data.len())).collect(), stroke));

        if scale > 0.6 {
            let mut x = first_data_ix as f32 + 0.5;
            let mut prev = wave_data[first_data_ix];
            let mut prev_start_x = x;
            let last_x = last_data_ix as f32 - 2.0;
            for &h in &wave_data[first_data_ix+1..last_data_ix] {
                if h == prev && x < last_x {
                    x += 1.0;
                    continue;
                }
